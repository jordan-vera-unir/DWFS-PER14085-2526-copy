...
function redConverter() {
 ...
 for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      pixels[i][j][1] = 0; // Canal Verde a 0
      pixels[i][j][2] = 0; // Canal Azul a 0
    }
  }
...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i += 2) {
    ...
    temporalPixels.push(nuevaFila);
  }
  pixels = temporalPixels;
...
function merge(alphaFirst, alphaSecond) {
 ...
 for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      let r = (catPixels[i][j][0] * alphaFirst) + (dogPixels[i][j][0] * alphaSecond);
      let g = (catPixels[i][j][1] * alphaFirst) + (dogPixels[i][j][1] * alphaSecond);
      let b = (catPixels[i][j][2] * alphaFirst) + (dogPixels[i][j][2] * alphaSecond);
      row.push([r, g, b]);
    }
    pixels.push(row);
  }
