...
function redConverter() {
    ...
    //Aqui tu codigo
    for (let i = 0; i < pixels.length; i++) { //recorre filas
        for (let j = 0; j < pixels[i].length; j++) { //recorre columnas
...
function greyConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) { //recorre filas
        for (let j = 0; j < pixels[i].length; j++) { //recorre columnas
            let pixel = pixels[i][j]; //pixel en cada posicion
            let media = Math.round(pixel.reduce((sum,num) => sum + num, 0)/pixel.length);
            pixels[i][j] = [media, media, media]; //asigna la media a los 3 canales
        }
    }
    handler.savePixels(pixels, outputPath);
}
...
function scaleDown() {
    ...
    //Aqui tu codigo
    let nuevosPixels  = []; //array para la imagen reducida

    for (let i = 0; i < pixels.length; i+=2) { //recorre solo filas pares
        let nuevaFila = []; //nueva fila
        for (let j = 0; j < pixels[i].length; j+=2) { //recorre solo columnas pares
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j][0] = pixels[i][j][0]/dimFactor; //r
            pixels[i][j][1] = pixels[i][j][1]/dimFactor; //g
            pixels[i][j][2] = pixels[i][j][2]/dimFactor; //b
        }
    }
    handler.savePixels(pixels, outputPath);
}
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < catPixels.length; i++) {
        ...
        for (let j = 0; j < catPixels[i].length; j++) {
            ...
            let r = pixelCat[0] * alphaFirst + pixelDog[0] * alphaSecond;
            let g = pixelCat[1] * alphaFirst + pixelDog[1] * alphaSecond;
            let b = pixelCat[2] * alphaFirst + pixelDog[2] * alphaSecond;
