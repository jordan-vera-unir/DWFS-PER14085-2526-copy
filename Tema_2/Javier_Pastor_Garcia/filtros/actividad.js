...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    if ((i % 2) !== 0) {
      ...
      for (let j = 0; j < pixels[i].length; j++) {
        if ((j % 2) !== 0) {
          pixelesreducido[auxi][auxj] = pixels[i][j];
          auxj++;
        }
      }
      auxi++;
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      let auxpixels = [];
      auxpixels[0] = Math.round(dogPixels[i][j][0] * alphaFirst + catPixels[i][j][0] * alphaSecond);
      auxpixels[1] = Math.round(dogPixels[i][j][1] * alphaFirst + catPixels[i][j][1] * alphaSecond);
      auxpixels[2] = Math.round(dogPixels[i][j][2] * alphaFirst + catPixels[i][j][2] * alphaSecond);
